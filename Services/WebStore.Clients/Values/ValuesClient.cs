...
namespace WebStore.Clients.Values
{
    ...
    {
        public ValuesClient(IConfiguration config) : base(config, "api/values") { } //адрес сопоставляется с адресом в контроллере


        // далее реализация методов из интерфейса IValueService
        ...
        {
            var response = await _Client.PostAsJsonAsync($"{_ServiceAddress}/post", value); //(куда отправляем, что отпр)
