...
namespace WebStore.Clients.Base
{
    public abstract class BaseClient : IDisposable
    {
        ...
        protected T Get<T>(int id) where T : new() => Get<T>(id.ToString());

        protected T Get<T>(string url = null) where T : new() => GetAsync<T>(Check(url)).Result;

        protected async Task<T> GetAsync<T>(int id, CancellationToken Cancel = default) where T : new() => await GetAsync<T>(id.ToString(), Cancel);
        ...
        protected HttpResponseMessage Post<T>(int id, T item) => Post(id.ToString(), item);

        protected HttpResponseMessage Post<T>(T item) => Post(_ServiceAddress, item);

        protected HttpResponseMessage Post<T>(string url, T item) => PostAsync(url, item).Result;

        protected async Task<HttpResponseMessage> PostAsync<T>(int id, T item, CancellationToken Cancel = default) => await PostAsync(id.ToString(), item, Cancel);
        ...
        protected HttpResponseMessage Put<T>(int id, T item) => PutAsync(id.ToString(), item).Result;
        protected HttpResponseMessage Put<T>(T item) => PutAsync(_ServiceAddress, item).Result;
        protected HttpResponseMessage Put<T>(string url, T item) => PutAsync(url, item).Result;

        protected async Task<HttpResponseMessage> PutAsync<T>(int id, T item, CancellationToken Cancel = default) => await PutAsync(id.ToString(), item, Cancel);
        ...
        protected HttpResponseMessage Delete(int id) => DeleteAsync(id.ToString()).Result;
        protected HttpResponseMessage Delete(string url) => DeleteAsync(url).Result;

        protected async Task<HttpResponseMessage> DeleteAsync(int id, CancellationToken Cancel = default) => await DeleteAsync(id.ToString(), Cancel);
